...
import { Wand2, Loader2, Image as ImageIcon, AlertCircle, Download } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError("Something went wrong while creating your masterpiece. Please try again.");
    } finally {
  ...
  const handleDownloadCurrent = () => {
    ...
    link.download = `muse-creation-${currentImage.id}.jpg`;
          ...
          <div className="w-full lg:w-1/2 space-y-8">
            <div className="space-y-2">
              <h2 className="font-serif text-4xl md:text-5xl text-slate-900 leading-tight">
                What would you like <br/> to <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-500 to-purple-600">create today?</span>
              </h2>
              <p className="text-slate-500 text-lg font-light">Describe your vision and let Muse bring it to life.</p>
            </div>

            <form onSubmit={handleGenerate} className="space-y-6 bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-white/50">
              <div className="space-y-3">
                <label className="block text-sm font-semibold text-slate-700 uppercase tracking-wider">The Vision</label>
                <textarea
                  value={prompt}
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold text-slate-700 uppercase tracking-wider">Format</label>
                <AspectRatioSelector 
            ...
            `}>
              {currentImage ? (
                ...
                <div className="text-center p-12 space-y-4 max-w-xs mx-auto">
                  ...
                  <h3 className="text-slate-900 font-medium text-lg">Canvas Empty</h3>
                  <p className="text-slate-500">Your masterpiece will appear here once you start dreaming.</p>
                </div>
              )}
              ...
              {isGenerating && (
                <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                   ...
                   <p className="text-primary-700 font-medium animate-pulse">Creating magic...</p>
                </div>
              )}
            </div>
          </div>
