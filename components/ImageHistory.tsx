...
const ImageHistory: React.FC<ImageHistoryProps> = ({ images, onDelete }) => {
  ...
  const handleDownload = (image: GeneratedImage) => {
    ...
    link.href = image.url;
    link.download = `muse-creation-${image.id}.jpg`;
  ...
  return (
    <div className="mt-16 w-full max-w-5xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {images.map((img) => (
          <div key={img.id} className="group relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-slate-100">
            ...
            <div className="p-4">
              <p className="text-sm text-slate-600 line-clamp-2 mb-3" title={img.prompt}>
                "{img.prompt}"
              </p>
